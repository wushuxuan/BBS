...
let allUsers = function() {
    let sql = `
    select
    user.id,user.account,user.name,user.imgSrc
    from
    user
    order by
    user.id desc
...
//创建新用户
let register = function(passage) {
        let sql = `
        insert into
            user (account,password,name,signature,breif)
        values
            ('${passage.account}','${passage.password}','${passage.name}',"","");
        `;
        return sql;
    }
    //前端sql语句
...
let allArticleFront = function() {
    let sql = `
        select
            article.id,article.title,article.time,article.category,article.tags,top,hot,zan,createid,article.summary,article.content
        from
            article
        where
            article.state='发表'
...
//获取所有文章（完成）
let UserArticleFront = function(id) {
    let sql = `
        select
            article.id,article.title,article.time,article.category,article.tags,top,hot,zan,createid,article.summary,article.content
        from
            article
        where
            article.state='发表' and article.createid = ${id}
        order by
            article.time desc ,article.id desc
        `;
    return sql;
...
//新增留言
   let makeMessage = function(comment) {
    let sql = `
    insert into 
        messages (createid,content) 
    values 
...
let getMessage = function() {
    let sql = `
        ...
        from
            messages 
        order by 
        messages.time desc ,messages.id desc
        `;
    return sql;
}
...
let createArticle = function(passage) {
    let sql = `
        insert into
            article (title,time,category,tags,top,hot,zan,createid,state,summary,content)
        values
            ('${passage.title}','${passage.time}','${passage.category}','${passage.tags}','1','1',0,'${passage.createid}','${passage.state}','${passage.summary}','${passage.content}');
        `;
    return sql;
}
