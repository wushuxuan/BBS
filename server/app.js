...
app.use(expressJWT({
    ...
    path: ['/api/user/login', '/api/user/register', '/api/user/getInfo', '/ueditor/ue'] //这里可以设置保护路由，login就不用进行token验证
}));
...
app.use('/*', function(err, req, res, next) {
    ...
    if (originalUrl != '/api/user/register' && originalUrl != '/api/user/upload' && originalUrl != '/api/user/getFileList' && originalUrl.indexOf('/api/user') > -1 || originalUrl.indexOf('/api/backend') > -1) {
